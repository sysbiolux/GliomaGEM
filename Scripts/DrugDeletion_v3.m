function [grRatio, grRateKO, grRateWT, hasEffect, delRxns, fluxSolution] = DrugDeletion_v3(model, method, DrugList,GeneTargetPharma)
 % adding strtok(model.genes,'.') for selecting genes in the database
GeneTargetPharma = GeneTargetPharma(ismember(GeneTargetPharma.ENTREZ,strtok(model.genes,'.')),:);  
...
for i = 1:numel(DrugList)
    ...
    idx = ismember(GeneTargetPharma.DrugName, DrugList(i));
    geneList = cellstr(GeneTargetPharma.ENTREZ(idx));
    %geneList= model.genes(ismember(strtok(model.genes,'.'), geneList));
    %geneList= intersect(strtok(model.genes,'.'), geneList);
    [modelDel,hasEffect(i),constrRxnNames] = deleteModelGenes_rFASTCORMICS(model,geneList);
